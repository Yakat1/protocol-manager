...
export default function Workspace({ state, setState, updateState, activeSubjectId, setActiveSubjectId, onExportCSV, onExportBackup, onImportBackup }) {
  ...
  return (
    <div className="culture-container">
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px'}}>
          <div>
            <h2 style={{color: 'var(--text-primary)', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px'}}>
              <Users size={24} color="var(--accent)"/> Gestión de Sujetos
            </h2>
            <p className="no-print" style={{color: 'var(--text-secondary)', fontSize: '0.9rem'}}>Registra variables y adjunta evidencias por sujeto.</p>
          </div>
      ...
      {/* Vista Dividida: Sujetos - Detalles */}
      <div className="culture-split-view">
        ...
        <div className="culture-sidebar glass-panel no-print">
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px'}}>
            <h4 style={{display:'flex', alignItems:'center', gap:'6px'}}><Box size={16}/> Sujetos Actuales</h4>
            <button className="btn-icon" onClick={addSubject} title="Añadir Sujeto" style={{background: 'var(--accent)', color: 'white', borderRadius: '4px'}}><Plus size={16}/></button>
          </div>
          ...
          <div className="culture-list">
            {state.subjects.map(s => (
              ...
              >
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginTop: '6px'}}>
                  ...
                  <button className="btn-icon" onClick={(e) => removeSubject(s.id, e)} style={{color: 'var(--danger)', padding: 0}}><Trash2 size={14}/></button>
                </div>
            ...
            {state.subjects.length === 0 && <div className="empty-mini">No hay sujetos. Añade uno con el botón +.</div>}
          </div>
        </div>
        ...
        <div className="culture-timeline-view">
          {!subject ? (
             <div className="glass-panel" style={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', textAlign: 'center', height: '100%'}}>
               <Users size={64} color="var(--accent)" style={{opacity: 0.3, marginBottom: '20px'}}/>
               <h3 style={{color: 'var(--text-primary)', marginBottom: '8px'}}>Selecciona un Sujeto</h3>
               <p style={{color: 'var(--text-secondary)'}}>Elige un sujeto de la lista o crea uno nuevo para registrar sus variables y evidencia.</p>
               <button className="btn btn-primary" onClick={addSubject} style={{marginTop: '20px', padding: '10px 20px'}}>
